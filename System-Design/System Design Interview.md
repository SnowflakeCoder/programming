...
With the growth of the user base, we need multiple servers. Separating web/mobile traffic (web tier) and database (data tier) servers allows them to be **scaled independently**.

**Which databases to use?**

- **Relational databases** are also called a RDBMS or SQL database. The most popular ones are MySQL, Oracle database, PostgreSQL, etc. <u>Relational databases represent and store data in tables and rows</u>. You can perform join operations using SQL across different database tables.
- **Non-Relational databases** are also called **NoSQL databases**. Popular ones are CouchDB, Neo4j, Cassandra, HBase, Amazon DynamoDB, etc. These databases are grouped into **four categories**: key-value stores, graph stores, column stores, and document stores. <u>Join operations are generally not supported</u> in non-relational databases.
...
Vertical scaling, referred to as “**scale up**”, means the process of <u>adding more power</u> (CPU, RAM, etc.) to your servers. Horizontal scaling, referred to as “**scale-out**”, allows you to scale by <u>adding more servers</u> into your pool of resources. When traffic is low, vertical scaling is a great option, and the <u>simplicity of vertical scaling is its main advantage</u>. 
...
recrawl downloaded pages to keep our data set fresh. Recrawl all the URLs is timeconsuming
and resource intensive. Few strategies to optimize freshness are listed as follows:
...
40. Third-party services send notifications to user devices.
      Step 3 - Design deep dive
      ...
      [5] You Cannot Have Exactly-Once Delivery: https://bravenewgeek.com/you-cannot-haveexactly-
      once-delivery/
...
hydrated news feed.
51. The fully hydrated news feed is returned in JSON format back to the client for
      rendering.
      Cache architecture
...
60. Chat server 2 forwards the message to User B. There is a persistent WebSocket
      ...
      Congratulations on getting this far! Now give yourself a pat on the back. Good job!
      Reference materials
      [1] Erlang at Facebook: https://www.erlangfactory.
      com/upload/presentations/31/EugeneLetuchy-ErlangatFacebook.pdf
      [2] Messenger and WhatsApp process 60 billion messages a day:
      https://www.theverge.com/2016/4/12/11415198/facebook-messenger-whatsapp-numbermessages-
      vs-sms-f8-2016
      [3] Long tail: https://en.wikipedia.org/wiki/Long_tail
      [4] The Underlying Technology of Messages: https://www.facebook.com/notes/facebookengineering/
      the-underlying-technology-of-messages/454991608919/
      [5] How Discord Stores Billions of Messages: https://blog.discordapp.com/how-discordstores-
      billions-of-messages-7fa6ec7ee4c7
      [6] Announcing Snowflake: https://blog.twitter.com/engineering/en_us/a/2010/announcingsnowflake.
      html
   ...
   Step 3 - Design deep dive
   In the high-level design, we discussed data gathering service and query service. The highlevel
   design is not optimal, but it serves as a good starting point. In this section, we will dive
...
the client.
73. In case the data is not in Trie Cache, we replenish data back to the cache. This way, all
      ...
      results are intended for a single user and must not be cached by a shared cache. “maxage=
      3600” means the cache is valid for 3600 seconds, aka, an hour.
      ...
      level. For example, data queries that start with ‘a’ can be split into 4 servers: ‘aa-ag’, ‘ahan’,
      ‘ao-au’, and ‘av-az’.
      ...
      https://medium.com/@prefixyteam/how-we-built-prefixy-a-scalable-prefix-search-servicefor-
...
88. Build your own CDN like Netflix and partner with Internet Service Providers (ISPs).
...
16. Client 2 downloads all the new blocks to reconstruct the file.
